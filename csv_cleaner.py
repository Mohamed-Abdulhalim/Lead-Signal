...
def normalize_photo_identity(u):
    ...
    if not HTTP_RE.search(u):return "",""
    base=re.sub(r"[\?].*$","",u)
    base=base.split("=",1)[0]
    low=base.lower()
    return base,low
...
def clean_and_dedupe_photos(raw):
    if not raw:return ""
    parts=[p.strip() for p in raw.split(",") if p.strip()]
    out=[]
    seen=set()
    for p in parts:
        base,_=normalize_photo_identity(p)
...
def process(in_path, out_path, drop_empty_name=False):
    ...
    for row in rows:
        for k in list(row.keys()):
            row[k] = nfc(row[k])
        name_val=nfc(row.get("name",""))
        if not name_val or name_val.lower() in ("nan","iraq"):
            continue
        ...
        if not addr_rec:
            addr_src = "empty"
        row["address_line"] = normalize_separators(addr_rec)
        row["category_line"] = normalize_separators(row.get("category_line",""))
        row["price_text"] = ptxt
        ...
        if "photo_urls" in row:
            _p=clean_and_dedupe_photos(row.get("photo_urls",""))
            row["photo_urls"]=_p.split(",")[0].strip() if _p else ""
        k = dedupe_key(row)
        if k in seen:
            continue
        seen.add(k)
